...
export default function Header({ onMenuClick }) {
  ...
  return (
    <header 
      className="h-14 border-b flex items-center justify-between px-3 sm:px-4 lg:px-6 flex-shrink-0"
    ...
    >
      <div className="flex items-center gap-2 sm:gap-3 min-w-0">
        ...
        <button 
          onClick={onMenuClick}
          className="lg:hidden p-2 rounded-xl text-slate-400 hover:text-white transition-all border"
          ...
          {breadcrumbs.map((crumb, i) => (
            <span key={i} className="flex items-center gap-2 flex-shrink min-w-0 animate-in slide-in-from-left-2 duration-300">
              ...
              {crumb.to ? (
                <Link 
                  to={crumb.to} 
                  className="text-slate-500 hover:text-slate-200 transition-all font-medium px-2 py-1 rounded-lg truncate max-w-[9rem] sm:max-w-none hover:bg-opacity-50"
                  style={{ backgroundColor: 'transparent' }}
                >
                  {crumb.label}
                </Link>
              ) : (
                <span className="font-bold px-2 py-1 truncate max-w-[10rem] sm:max-w-none" style={{ color: 'var(--text-main)' }}>{crumb.label}</span>
              )}
            </span>
          ))}
          {isViewer && (
      ...
      <div className="flex items-center gap-2 sm:gap-4 pl-2">
        ...
        <div 
          className="hidden sm:flex items-center gap-2.5 border rounded-xl px-4 py-1.5 transition-all cursor-default group"
          style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-main)' }}
        >
          ...
          <span className="font-bold text-base tracking-tight" style={{ color: 'var(--text-main)' }}>TaskForge</span>
        </div>

        <div className="flex items-center gap-2 sm:gap-4 border-l border-slate-800 pl-2 sm:pl-4 group">
